...
import { Role } from "../../../drizzle/types/Role";
...
export default async function ResumePage() { // Changed to async function
  const history: Role[] = await db.select().from(roles).orderBy(desc(roles.startDate))
    ...
    <div className="bg-glass rounded-xl p-10 min-h-screen">
      ...
      <Timeline>
        {history.map((item:Role, index) => (
          ...
          >
            <div className="">{dayjs(item.startDate).format("YYYY MMM")}{item.endDate && <>- {dayjs(item.endDate).format("YYYY MMM")}</>}</div>
